...
impl FixedPredictor {
    ...
    pub fn best_predictor_order(data: Vec <i32>) -> Option <u32> {
        ...
        for order in 0..=4 {
            if let Some(residuals) = FixedPredictor::get_residuals(data.clone(), order) {
                let sum_residuals = FixedPredictor::get_sum(&residuals);
                if sum_residuals < min_sum {
                    min_sum = sum_residuals;
    ...
    fn get_sum(residuals: &Vec <i32>) -> i64 {
